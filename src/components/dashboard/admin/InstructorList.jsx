...
const InstrcutorView = () => {
  ...
  useEffect(() => {
    ...
    const fetchStudents = async () => {
      try {
        ...
        setInstructors(response.data.registeredteachers || []);
        setFilteredStudents(response.data.registeredteachers || []);
      } catch (error) {
        setError(
          error.response?.data?.message || "Error fetching Instructor data"
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setInstructors(instructors.filter((student) => student.id !== id));
        setFilteredStudents(
          filteredStudents.filter((student) => student.id !== id)
        );
      } else {
        alert(`Error: ${response.data.msg || "Failed to delete instructor"}`);
      }
    } catch (error) {
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      {loading ? (
        <p className="text-center text-gray-500">Loading instructor...</p>
      ) : error ? (
          ...
          <table className="min-w-full bg-gray-900 text-white shadow-lg rounded-lg">
            <thead>
              <tr className="bg-gradient-to-r from-purple-600 to-blue-500 text-white">
                ...
                <th className="py-3 px-6 text-left">Remove Instructor</th>
              </tr>
            </thead>
            <tbody>
