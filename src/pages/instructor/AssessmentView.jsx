...
import InstructorAssessmentCard from "../../components/courses/InstructorAssessment";
import { Plus } from "lucide-react";
import { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import axios from "axios";

export default function MyAssessmentALL() {
          ...
          <div className="mb-8 flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-white mb-2">
                My Uploaded Assessments
              </h1>
              <p className="text-gray-400">Your uploaded Assessments</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCourses.length > 0 ? (
              filteredCourses.map((course) => (
                <InstructorAssessmentCard key={course.id} {...course} />
              ))
            ) : (
